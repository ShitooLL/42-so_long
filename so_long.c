...
/*   Updated: 2021/12/21 21:28:53 by laliao           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_display_texture(t_render *render,t_game *game, int x, int y)
{
	if (game->map_data[y][x] == '1')
		mlx_put_image_to_window(render->mlx, render->window, game->img_1, x, y);			
	else if (game->map_data[y][x] == '0')
		mlx_put_image_to_window(render->mlx, render->window, game->img_0, x, y);
	else if (game->map_data[y][x] == 'C')
		mlx_put_image_to_window(render->mlx, render->window, game->img_c, x, y);
	else if	(game->map_data[y][x] == 'E')
		mlx_put_image_to_window(render->mlx, render->window, game->img_e, x, y);
}
